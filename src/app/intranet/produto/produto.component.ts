...
export class ProdutoComponent implements OnInit, AfterViewInit {
  ...
  montaForm() {
    this.formProduto = this.formBuilder.group({
      ...
      idLinha: [this.produto.idLinha, Validators.required],
  ...
  salvar() {
    if (this.formProduto.valid && !this.formProduto.pending) {
      ...
      this.produto.preco = modelForm.preco;
      this.produto.ordem = modelForm.ordem;
  ...
  copiarProduto(event) {
    this.loadingService.show();
    this.adminProdutoService.copiarProduto(this.produto, event).subscribe(
      res => {
        if (res) {
          this.loadingService.hide();
