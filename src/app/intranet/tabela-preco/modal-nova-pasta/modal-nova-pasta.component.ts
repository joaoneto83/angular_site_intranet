...
export class ModalNovaPastaComponent implements OnInit {
    ...
    pasta: PastaTabelaPreco = {} as PastaTabelaPreco;
    closeResult: string;
    ...
    salvar() {
        if (this.formTabela.valid && !this.formTabela.pending) {
            ...
            const pasta = this.formTabela.getRawValue() as PastaTabelaPreco;

            this.pasta = pasta;
            this.pasta.idPai = this.idPai;

            this.tabelaService.postPastaEditar(this.pasta).subscribe(
