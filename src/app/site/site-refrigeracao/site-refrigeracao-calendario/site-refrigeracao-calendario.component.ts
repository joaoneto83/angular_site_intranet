...
@Component({
    selector: 'app-site-refrigeracao-calendario',
    templateUrl: './site-refrigeracao-calendario.component.html',
    styleUrls: ['./site-refrigeracao-calendario.component.css']
})
export class SiteRefrigeracaoCalendarioComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.eventoService.getTodosEventos(this.cultura).subscribe(
    ...
    carregarEventos(tipo: number, mes: number) {
        ...
        this.eventoService.getEventos(tipo, mes, this.cultura).subscribe(
            ret => this.getEventosSuccess(ret),
            err => this.getEventosError(err)
        );
    }
